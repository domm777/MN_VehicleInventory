...
using MN_VehicleInventory.Domain.Aggregate;
using MN_VehicleInventory.Domain.Enums;

namespace MN_VehicleInventory.Application.Services {
    public class MN_VehicleService : MN_IVehicleService {
        private readonly MN_IVehicleRepository _vehicleRepository;

        public MN_VehicleService(MN_IVehicleRepository vehicleRepository) {
        ...
        public async Task<MN_VehicleDto?> GetVehicleById(int id) {
        ...
        public async Task UpdateVehicleStatus(int id, string newStatus) {
            var vehicle = await _vehicleRepository.GetVehicleById(id);
